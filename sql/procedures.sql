...
CREATE PROCEDURE get_property_details(IN p_id INT)
BEGIN
    SELECT 
        p.*,
        CASE 
...
BEGIN
    ...
    WHERE 
        ...
        AND (property_type IS NULL OR 
            (property_type = 'Residential' AND r.property_id IS NOT NULL) OR
            (property_type = 'Commercial' AND c.property_id IS NOT NULL))
        AND (min_beds IS NULL OR r.bedrooms >= min_beds)
        AND (min_baths IS NULL OR r.bathrooms >= min_baths)
...
BEGIN
    SELECT 
        ...
        SUM(CASE WHEN t.transaction_type = 'Sale' THEN t.amount ELSE 0 END) as total_sales_volume,
        SUM(CASE WHEN t.transaction_type = 'Lease' THEN t.amount ELSE 0 END) as total_lease_volume,
        SUM(t.commission_amount) as total_commission,
        AVG(CASE WHEN t.transaction_type = 'Sale' THEN t.amount ELSE NULL END) as avg_sale_price,
        COUNT(DISTINCT 
            CASE WHEN t.transaction_type = 'Sale' 
            THEN t.transaction_id END) as num_sales,
        COUNT(DISTINCT 
            CASE WHEN t.transaction_type = 'Lease' 
            THEN t.transaction_id END) as num_leases
    FROM Agent a
    LEFT JOIN AgentListing al ON a.agent_id = al.agent_id
        AND (start_date IS NULL OR al.listing_date >= start_date)
        AND (end_date IS NULL OR al.listing_date <= end_date)
    LEFT JOIN AgentShowing ash ON a.agent_id = ash.agent_id
        AND (start_date IS NULL OR ash.showing_date >= start_date)
        AND (end_date IS NULL OR ash.showing_date <= end_date)
...
BEGIN
    ...
    GROUP BY c.client_id, c.client_name, c.client_phone, c.client_email;
END //
...
BEGIN
    DECLARE existing_listing INT;
    
    -- Check if property already has an active listing
    SELECT listing_id INTO existing_listing
    FROM AgentListing
    WHERE property_id = p_property_id
        AND expiration_date > CURRENT_DATE();
        
    IF existing_listing IS NOT NULL THEN
        SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = 'Property already has an active listing';
    END IF;
...
BEGIN
    ...
    SET p_showing_id = LAST_INSERT_ID();
END //
...
BEGIN
    SELECT 
        ...
        SUM(CASE WHEN t.transaction_type = 'Lease' THEN 1 ELSE 0 END) as total_leases,
        AVG(CASE WHEN t.transaction_type = 'Sale' THEN t.amount END) as avg_sale_price,
        AVG(CASE WHEN t.transaction_type = 'Lease' THEN t.amount END) as avg_lease_price,
        MIN(CASE WHEN t.transaction_type = 'Sale' THEN t.amount END) as min_sale_price,
        MAX(CASE WHEN t.transaction_type = 'Sale' THEN t.amount END) as max_sale_price,
    ...
    WHERE 
        ...
        AND (property_type IS NULL 
             OR (property_type = 'Residential' AND r.property_id IS NOT NULL)
             OR (property_type = 'Commercial' AND c.property_id IS NOT NULL));
END //
